...
if ($usrcanjoin and confirm_sesskey($sesskey)) {
    ...
    $meetingserver = isset($_REQUEST['meetingserver']) && $_REQUEST['meetingserver'] && is_numeric($_REQUEST['meetingserver']) ? $_REQUEST['meetingserver'] : ($DB->get_field('adobeconnect', 'meetingserver', ['id' => $cm->instance]) ?: 1);
    ...
    /*if ($meetingscoid && $usrprincipal && empty($meeting)) {
        $meeting = aconnect_create_meeting($aconnect, $meetfldscoid, $meetfldscoid);
...
//                 die();
    ...
    if (empty($validuser)) {
        ...
        /*var_dump($aconnect->request_http_header_login(1, $login));
        die();*/
