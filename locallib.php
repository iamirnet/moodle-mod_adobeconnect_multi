...
/*require_once($CFG->dirroot . '/mod/adobeconnect/mod_form.php');*/
 ...
 */
function aconnect_login($meetingserver = null)
{
    global $CFG, $USER, $COURSE, $DB;
    if (!$meetingserver)$meetingserver = isset($_REQUEST['meetingserver']) && $_REQUEST['meetingserver'] && is_numeric($_REQUEST['meetingserver']) ? $_REQUEST['meetingserver'] : ($DB->get_field('adobeconnect', 'meetingserver', ['course' => $COURSE->id]) ?: 1);
    /*var_dump($meetingserver);
    die();*/
    ...
    $configac = (object) [
        ...
        "adobeconnect_admin_login" => $CFG->{"adobeconnect_{$meetingserver_prefix}admin_login"},
        "adobeconnect_admin_password" => $CFG->{"adobeconnect_{$meetingserver_prefix}admin_password"},
    ];
    if (!isset($configac->adobeconnect_host) or
        !isset($configac->adobeconnect_admin_login) or
        !isset($configac->adobeconnect_admin_password)) {
    ...
    if (isset($configac->adobeconnect_port) and
        !empty($configac->adobeconnect_port) and
        ((80 != $configac->adobeconnect_port) and (0 != $configac->adobeconnect_port))) {
    ...
    $aconnect = new connect_class_dom($configac->adobeconnect_host,
        $configac->adobeconnect_port,
        $configac->adobeconnect_admin_login,
        $configac->adobeconnect_admin_password,
        '',
        $https);
                    ...
                    if ($innernodelist->item($x)->hasAttributes()) {
                        ...
                        if (!is_null($domnode)) {
                            ...
                            if (!is_null($meetingdetail->attributes->getNamedItem('duration')->nodeValue)) {
                                ...
                                $value = $meetingdetail->attributes->getNamedItem('duration')->nodeValue;
