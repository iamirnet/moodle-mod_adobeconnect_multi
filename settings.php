...
if ($ADMIN->fulltree) {
    ...
    if ($_REQUEST['server']) {
        $server = $_REQUEST['server'] == 1 ? "" : "s_{$_REQUEST['server']}_";

        $settings->add(new admin_setting_configtext("adobeconnect_{$server}host", get_string('host', 'adobeconnect'),
            get_string('host_desc', 'adobeconnect'), 'localhost/api/xml', PARAM_URL));

        $settings->add(new admin_setting_configtext("adobeconnect_{$server}meethost", get_string('meetinghost', 'adobeconnect'),
            get_string('meethost_desc', 'adobeconnect'), 'localhost', PARAM_URL));

        $settings->add(new admin_setting_configtext("adobeconnect_{$server}port", get_string('port', 'adobeconnect'),
            get_string('port_desc', 'adobeconnect'), '80', PARAM_INT));

        $settings->add(new admin_setting_configtext("adobeconnect_{$server}admin_login", get_string('admin_login', 'adobeconnect'),
            get_string('admin_login_desc', 'adobeconnect'), 'admin', PARAM_TEXT));

        $settings->add(new admin_setting_configpasswordunmask("adobeconnect_{$server}admin_password", get_string('admin_password', 'adobeconnect'),
            get_string('admin_password_desc', 'adobeconnect'), ''));

        $settings->add(new admin_setting_configtext("adobeconnect_{$server}admin_httpauth", get_string('admin_httpauth', 'adobeconnect'),
            get_string('admin_httpauth_desc', 'adobeconnect'), 'my-user-id', PARAM_TEXT));

        $settings->add(new admin_setting_configcheckbox("adobeconnect_{$server}email_login", get_string('email_login', 'adobeconnect'),
            get_string('email_login_desc', 'adobeconnect'), '0'));

        $settings->add(new admin_setting_configcheckbox("adobeconnect_{$server}https", get_string('https', 'adobeconnect'),
            get_string('https_desc', 'adobeconnect'), '0'));

        $str = '<input type="hidden" name="server"  value="'.$_REQUEST['server'].'" />';
        $settings->add(new admin_setting_heading('adobeconnect_server', '', $str));

        $url = $CFG->wwwroot . '/mod/adobeconnect/conntest.php'. ($_REQUEST['server'] == 1 || !$_REQUEST['server'] ? "" : "?server={$_REQUEST['server']}");
        ...
        $settings->add(new admin_setting_configtext('adobeconnect_count_servers', get_string('count_servers', 'adobeconnect'),
            get_string('count_servers', 'adobeconnect'), '1', PARAM_TEXT));
        for($i = 0; $i < (get_config(NULL, 'adobeconnect_count_servers') ?: 1);$i++) {
            $url =$_SERVER['SCRIPT_URI'] . "?" . http_build_query(array_merge($_GET, ['server' => $i + 1]));
            $str = '<a href="'.$url.'">'.get_string('setting_server', 'adobeconnect') . ' ' . ($i + 1).'</a>';

            $settings->add(new admin_setting_heading('setting_server_link_'. ($i + 1), '', $str));
        }
    }

    $param = new stdClass();
    $param->image = $CFG->wwwroot.'/mod/adobeconnect/pix/rl_logo.png';
    $param->url = 'https://github.com/hossein142001/moodle-mod_adobeconnect';

    $settings->add(new admin_setting_heading('adobeconnect_intro', '', get_string('settingblurb', 'adobeconnect', $param)));
}
/*
$CFG->adobeconnect_meeting_servers = array(
  array('host' => 'el2.jsharif.com', 'port' => '443', 'username' => 'mehran_rashidzad@yahoo.com', 'password' => 'Hokmehran1'),
  array('host' => 'el3.jsharif.com', 'port' => '443', 'username' => 'mehran_rashidzad@yahoo.com', 'password' => 'Hokmehran1')
);*/
